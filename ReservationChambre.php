...
<h1><center> Formulaire de réservation  d'une chambre </center></h1>
    <div class="messageErreur" >
							<?php
								...
								if(isset($_GET['dispo'])) echo '<p> Nous sommes navrés, aucune chambre de ce type n\'est disponible durant cette date ☹</p>';
 
							
								if(isset($_GET['password']))if($_GET['password']) echo '<p>* Le mot de passe est éroné</p>';
								if(isset($_GET['email']))if($_GET['email']) echo '<p>* L\'email existe déja</p>';
								if(isset($_GET['pseudo'])) if($_GET['pseudo'])echo '<p>* Le pseudo n\'est pas disponible</p>';
							
							?>
     </div>
<form method="post" action="ajoutReservation.php" class="formulaire">
...
<legend> <h2> Préférences : </h2></legend>  
    ...
    <center><input type="submit" class="envoyer" value="Réserver"/></center>
